...
IRCGui::IRCGui(CreateInterfaceFn interfaceFactory) throw(std::runtime_error) : panel_(NULL), vguiEngine_(NULL), vguiScheme_(NULL)
{
...
void IRCGui::CreatePanel(void) throw(std::logic_error) {
	...
	panel_ = new CIRCPanel(vguiEngine_->GetPanel( PANEL_GAMEUIDLL ), vguiEngine_, vguiScheme_);
}
